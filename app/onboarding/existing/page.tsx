...
export default function ExistingUserOnboarding() {
  ...
  const handleMaxesChange = (lift: keyof Maxes) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleComplete = () => {
    const valuesToStore = maxType === 'actual' 
      ? Object.entries(maxes).reduce((acc, [lift, value]) => ({
          ...acc,
          [lift]: value * 0.9,
        }), {} as Maxes)
      : maxes;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-2xl mx-auto space-y-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Enter Your Maxes</h1>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {Object.entries(maxes).map(([lift, value]) => (
            <div key={lift} className="bg-white p-4 rounded-lg shadow-sm">
              <label className="block">
                ...
                <div className="relative mt-1">
                  <input
                    type="number"
                    value={(() => {
                      const baseValue = maxType === 'actual' ? value / 0.9 || '' : value || '';
                      const raw = weightUnit === 'kg' ? kgToLbs(Number(baseValue)) : baseValue;
                      return Math.round(raw);
                    })()}
                    onChange={handleMaxesChange(lift as keyof Maxes)}
              ...
              {maxType === 'actual' && value > 0 && (
                <div className="mt-2 text-sm text-gray-600">
                  Training Max: <WeightDisplay weight={maxes[lift as keyof Maxes]} />
                </div>
              )}
            </div>
          ))}
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
          <h3 className="font-medium text-blue-900">Your Training Maxes</h3>
          <div className="mt-2 grid sm:grid-cols-2 gap-4">
            {Object.entries(maxes).map(([lift, value]) => (
              <div key={lift} className="flex justify-between">
                <span className="capitalize text-blue-800">{lift}</span>
                <WeightDisplay weight={value} className="font-medium text-blue-900" />
              </div>
            ))}
          </div>
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {Object.keys(maxes).map((lift) => (
                <button
                  key={lift}
                  onClick={() => dispatch(setCurrentLift(lift as Lift))}
